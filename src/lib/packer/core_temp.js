                                ...
                                while (curZ < H && zPossible) {
                                    ...
                                    for (var _g = 0, allProducts_1 = allProducts; _g < allProducts_1.length; _g++) {
                                        ...
                                        var _loop_3 = function (to) {
                                            ...
                                            if (to.type === 'lay' && to.w === 935)
                                                console.log("lay w=935:", "fitCountW", fitCountW, "fitCountL", fitCountL, "combinedAvail", combinedAvail, "rowCount", rowCount);
                                            var rowItems = [];
                                            var placedCount = 0;
                                            var currentIdx = 0;
                                            var qtyUsed = 0;
                                            for (var riL = 0; riL < fitCountL && placedCount < rowCount; riL++) {
                                                for (var riW = 0; riW < fitCountW && placedCount < rowCount; riW++) {
                                                    ...
                                                    // 1. 공중 부양 방지 지탱면 검사
                                                    var supp = hasSupportAtZInTemp(targetX, targetYRel, to.w, to.l, curZ, tempItems);
                                                    if (to.type === 'lay' && to.w === 935)
                                                        console.log("targetX:", targetX, "targetYRel:", targetYRel, "supp:", supp);
                                                    if (!supp) {
                                                        continue;
                                                    }
